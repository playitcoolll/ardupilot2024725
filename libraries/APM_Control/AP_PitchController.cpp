...
float AP_PitchController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, float aspeed,bool ground_mode)
{
    ...
    float R[3][3] = {{cosf(aoa_rad)*cosf(ssa_rad), -cosf(aoa_rad)*sinf(ssa_rad), -sinf(aoa_rad)},
                     ...
                     {sinf(aoa_rad)*cosf(ssa_rad), -sinf(aoa_rad)*cosf(ssa_rad),  cosf(aoa_rad)}};
    Vector3f ab = _ahrs.get_accel();//机体坐标系
    Vector3f ab2hangji = multiplyMatrixVector(R,ab);
 
    // Vector3f Ve;//计算航迹角
    // if(_ahrs.get_velocity_NED(Ve)){
    //     float flight_path_angle_rad = atan2(-Ve.z, sqrt(Ve.x * Ve.x + Ve.y * Ve.y));
    //     float flight_path_angle_deg = ToDeg(flight_path_angle_rad);
    //     // hal.console -> printf("%f\n",flight_path_angle_deg);
    // }
    Vector3f g_g = {0,0,9.81};
    Matrix3f dcm = _ahrs.get_DCM_rotation_body_to_ned();
    Vector3f g2ahangji = multiplyMatrixVector2(dcm,g_g);
    // hal.console -> printf("x=%f,y=%f,z=%f\n",g2ahangji.x+ab2hangji.x, g2ahangji.y+ab2hangji.y, g2ahangji.z+ab2hangji.z);
    float boost_az= 2.5;//不同速度下线化 根轨迹求出的结果
    if(aspeed >= 10){
        boost_az =2.5-(0.11*(aspeed - 10));
    }
    float out_az = boost_az * (g2ahangji.z+ab2hangji.z);
    ...
    boost_aoa = 1.66;//不同速度下线化 根轨迹求出的结果
    if(aspeed >= 10){
