...
float AP_RollController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, bool ground_mode)
{
    ...
    //计算P
    // float boost_roll = 0.15 * abs(_ahrs.roll_sensor * 0.01 - 0) + 0.15;
    // constrain_float(boost_roll , 0.15, 6);
    ...
    if (underspeed) {
        //低速减小D和P
        out_my -= D_term + 0.75 * out_my_p + I_term;
    }
    ...
    // tm = AP_HAL::millis();
    // if(tm - last_tm >5000){
    //     hal.console -> printf("%f1111111111111111\n",_last_out * 100);
    //     last_tm = tm;
    // }
