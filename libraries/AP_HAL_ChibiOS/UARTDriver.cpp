...
{
    //波特率为 0 可以用来标识 UART 目前不用于活动通信。这种状态下，UART 可能处于一种等待或待机状态，而不进行实际的数据传输
    if (b == 0 && txS == 0 && rxS == 0 && _tx_initialised && _rx_initialised) 
    {
        // just changing port owner
        _uart_owner_thd = chThdGetSelfX(); // 如果波特率、发送和接收缓冲区大小都为 0，并且发送和接收已经初始化，则仅更改端口所有者线程，而不重新初始化 UART
        return;
    }

    thread_rx_init();//初始化接收线程，以便能够正确地接收数据

    if (sdef.serial == nullptr) {
        return;//检查 sdef.serial 是否为空指针，如果为空则返回
    }
    uint16_t min_tx_buffer = HAL_UART_MIN_TX_SIZE;
    uint16_t min_rx_buffer = HAL_UART_MIN_RX_SIZE;

    /*
        根据波特率增加最小接收缓冲区大小，以确保在40Hz接收循环时能够完全利用 UART。
        这意味着 25 毫秒的数据。假设每字节 10 位，这是大多数协议的正常情况。
     */
    ...
    //如果识别到是串口4（迎角侧滑角传感器）那么特别设置以下存储大小
    if (this == hal.serial(4)) {
    ...
    /*
      ...
      在成功打开设备之前分配读取缓冲区，以便在系统启动的早期阶段进行分配，从而减少系统运行时堆内存的碎片化。这对于一些设备（如 ttyACM0 驱动）可能在启动后需要一段时间才能连接的情况特别重要。
     */
    while (_in_rx_timer) {
    ...
    }//检查当前的读取缓冲区大小是否与期望的大小 rxS 不同。如果不同，则调整缓冲区大小。
    ...
    if (clear_buffers) {
        _readbuf.clear();
    }//如果 clear_buffers 为 true，则清除读取缓冲区 _readbuf

#ifndef HAL_UART_NODMA//条件编译指令检查是否未定义 HAL_UART_NODMA，如果未定义，则启用以下代码段            Direct Memory Access，直接内存访问
    if (!half_duplex && !(_last_options & OPTION_NODMA_RX)) //不是半双工模式。未设置不使用 DMA 接收选项
    {
    ...
    /*
     展示了如何在初始化 UART 设备时分配写缓冲区（write buffer）
     */
    while (_in_tx_timer) {
        hal.scheduler->delay(1);
    }
    ...
    if (sdef.is_usb) {
        ...
        /*
         这段代码展示了如何根据设备类型（USB 或非 USB）初始化 UART 或 USB CDC 驱动，
         以及如何配置 DMA（直接内存访问）进行数据传输。在支持 USB 串行通信的情况下，
         会初始化一个 USB CDC 驱动；在非 USB 设备的情况下，会配置 UART 设备的 DMA 传输。
         */
